...
struct llama_file::impl {
    ...
    impl(const char *                fname,
         const char *                mode,
         [[maybe_unused]] const bool use_direct_io = false,
    ...
    size_t tell() const {
        ...
        return li.QuadPart - file_offset;
    }
    ...
    void seek(size_t offset, int whence) const {
        ...
        LARGE_INTEGER li;
        if (whence == SEEK_SET) {
            li.QuadPart = (LONGLONG) (file_offset + offset);
        } else if (whence == SEEK_END) {
            li.QuadPart = (LONGLONG) (file_offset + size + offset);
        } else {
    ...
    void write_raw(const void * ptr, size_t len) const {
        ...
        while (bytes_written < len) {
            ...
            BOOL   result        = WriteFile(fp_win32, reinterpret_cast<const char *>(ptr) + bytes_written, chunk_size,
                                             &chunk_written, NULL);
            if (!result) {
...
#    ifdef __linux__
        ...
        if (use_direct_io && std::strcmp(mode, "rb") == 0) {
            ...
            LLAMA_LOG_WARN("Failed to open file '%s' with error: %s. Falling back to buffered I/O", fname,
                           strerror(errno));
        }
#    endif
        init_fp(mode);
...
#    endif
    ...
    void init_fp(const char * mode) {
        ...
        size_t physical_size = (size_t) std::ftell(fp);
        size                 = physical_size - file_offset;
        if (file_offset > 0 && size >= 20) {
            size -= 20;  // Subtract BundleFooter size if we are in a bundle
        }
        std::fseek(fp, (long) file_offset, SEEK_SET);
    }
    ...
    size_t tell() const {
        ...
        return (size_t) pos - file_offset;
    }

    void seek(size_t offset, int whence) const {
        off_t absolute_offset = (off_t) offset;
        int   actual_whence   = whence;
        if (whence == SEEK_SET) {
            absolute_offset = (off_t) (file_offset + offset);
        } else if (whence == SEEK_END) {
            absolute_offset = (off_t) (file_offset + size + offset);
            actual_whence   = SEEK_SET;
        }

        off_t ret = 0;
        if (fd == -1) {
            ret = std::fseek(fp, (long) absolute_offset, actual_whence);
        } else {
            ret = lseek(fd, absolute_offset, actual_whence);
        }
        if (ret == -1) {
            throw std::runtime_error(format("seek error: %s", strerror(errno)));
        }
...
llama_file::llama_file(const char * fname, const char * mode, const bool use_direct_io, size_t file_offset) :
    pimpl(std::make_unique<impl>(fname, mode, use_direct_io, file_offset)) {}
...
size_t llama_file::offset() const {
    return pimpl->file_offset;
}

size_t llama_file::read_alignment() const {
    return pimpl->read_alignment();
}

bool llama_file::has_direct_io() const {
    return pimpl->has_direct_io();
}
...
#    endif
        ...
        size_t aligned_offset = (offset / pagesize) * pagesize;
        size_t offset_in_page = offset % pagesize;
        mmap_size             = size + offset_in_page;

        mmap_addr = mmap(NULL, mmap_size, PROT_READ, flags, fd, aligned_offset);
        if (mmap_addr == MAP_FAILED) {
            throw std::runtime_error(format("mmap failed: %s", strerror(errno)));
        }
        addr = (char *) mmap_addr + offset_in_page;
    ...
    impl(struct llama_file * file, size_t prefetch, bool numa) {
        ...
        SYSTEM_INFO si;
        GetSystemInfo(&si);
        size_t pagesize       = si.dwAllocationGranularity;
        size_t aligned_offset = (offset / pagesize) * pagesize;
        size_t offset_in_page = offset % pagesize;
        mmap_size             = size + offset_in_page;

        mmap_addr   = MapViewOfFile(hMapping, FILE_MAP_READ, (DWORD) (aligned_offset >> 32),
                                    (DWORD) (aligned_offset & 0xFFFFFFFF), mmap_size);
        DWORD error = GetLastError();
        ...
        for (int tries = 1;; tries++) {
            ...
            if (tries == 2) {
                LLAMA_LOG_WARN(
                    ...
                    size, llama_format_win_err(GetLastError()).c_str());
                return false;
            }
