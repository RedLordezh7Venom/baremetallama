        ...
        if (record.EventType == KEY_EVENT && record.Event.KeyEvent.bKeyDown) {
            ...
            if (wc == 0) {
                ...
                switch (record.Event.KeyEvent.wVirtualKeyCode) {
                    case VK_LEFT:
                        return ctrl_pressed ? KEY_CTRL_ARROW_LEFT : KEY_ARROW_LEFT;
                    case VK_RIGHT:
                        return ctrl_pressed ? KEY_CTRL_ARROW_RIGHT : KEY_ARROW_RIGHT;
                    case VK_UP:
                        return KEY_ARROW_UP;
                    case VK_DOWN:
...
static char32_t decode_utf8(const std::string & input, size_t pos, size_t & advance) {
    ...
    if ((c & 0xF0u) == 0xE0u && pos + 2 < input.size()) {
        ...
        advance = 3;
        return ((c & 0x0Fu) << 12) | ((static_cast<unsigned char>(input[pos + 1]) & 0x3Fu) << 6) |
    ...
    if ((c & 0xF8u) == 0xF0u && pos + 3 < input.size()) {
        ...
        advance = 4;
        return ((c & 0x07u) << 18) | ((static_cast<unsigned char>(input[pos + 1]) & 0x3Fu) << 12) |
...
static void move_word_left(size_t &                 char_pos,
                           size_t &                 byte_pos,
                           const std::vector<int> & widths,
                           const std::string &      line);
static void move_word_right(size_t &                 char_pos,
                            size_t &                 byte_pos,
                            const std::vector<int> & widths,
                            const std::string &      line);
static void move_to_line_start(size_t & char_pos, size_t & byte_pos, const std::vector<int> & widths);
static void move_to_line_end(size_t &                 char_pos,
                             size_t &                 byte_pos,
                             const std::vector<int> & widths,
                             const std::string &      line);
...
static void set_line_contents(std::string        new_line,
                              std::string &      line,
...
struct history_t {
    ...
    bool is_viewing() const { return viewing_idx != SIZE_MAX; }
} history;
    ...
    while (true) {
        ...
        if (input_char == '\033') {  // Escape sequence
            ...
            if (code == '[') {
                ...
                } else if ((code == '~' || (code >= 'A' && code <= 'Z') || (code >= 'a' && code <= 'z')) &&
                           !params.empty()) {
                    std::string digits;
                    for (char ch : params) {
                        if (ch == ';') {
